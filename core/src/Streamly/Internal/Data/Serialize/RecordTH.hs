        ...
        _ ->
            doE
                (bindS
                     ...
                     [|fmap w32ToInt <$>
                       deserialize $(varE n_finalOffOff) $(varE n_arr)|] :
                 mapWithLast
                     (makeBind False)
                     (makeBind True)
                     (zip [0 ..] fields) ++
  ...
  where
    ...
    skipField j bn@(DeserializeBindingNames {..}) field = do
        ...
        [|do let nextFieldOffOff =
             ...
             (_, nextFieldOff) <- deserialize nextFieldOffOff $(varE bnArr)
    ...
    ifTagEqExp (DeserializeBindingNames {..}) ty = do
        ...
        if isMType
            then [|let off = $(varE bnAfterTagOff) + 4
                    in do (off1, val) <- deserialize off $(varE bnArr)
                          pure (off1, Just val)|]
         ...
         in [|if $(varE bnOffset) >= $(varE bnFinalOff)
                  then pure ($(varE (makeI i)), $(nothingExp))
                      ...
                      ($(varP bnTagOff), $(varP bnTagLen)) <-
                          deserialize $(varE bnOffset) $(varE bnArr)
            ...
            , FunD
                  ...
                  [ Clause
                        (if isUnitType cons
                             ...
                             else [VarP n_initialOffset, VarP n_arr])
                        (NormalB peekMethod)
                        []
                  ]
            , FunD
